...
import trimesh
import numpy as np
from datasets.occluded import OccludedDataset
...
file_names = utils.get_occluded_filenames(os.path.join(meta_params["root_dir"]))
...
for ii, filename in enumerate(file_names):
    print(filename)
    basename = os.path.basename(filename).split(".")[0].split("_")[0]
    ...
    if os.path.isfile(recon_name):
        ...
        with h5py.File(filename, "r") as hf:
            gt_points = hf["gt_points"][:]

        # load ground truth data
        sdf_dataset = OccludedDataset(
