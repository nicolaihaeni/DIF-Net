...
class PointCloud_with_FreePoints(Dataset):
    def __init__(
        ...
        points, idx = points.squeeze(0).cpu().numpy(), idx.squeeze(0).cpu().numpy()
        self.farthest_coords = np.concatenate([points, self.normals(idx)], dim=-1)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "farthest_coords": torch.from_numpy(self.farthest_coords).float(),
        ...
        if isinstance(root_dir, list):
            ...
            for file in sorted(os.listdir(root_dir)):
                if file.endswith("mat"):
                    if os.path.isfile(
                        os.path.join(root_dir, file).replace(
                            "surface_pts_n_normal", "free_space_pts"
                        )
                    ):
